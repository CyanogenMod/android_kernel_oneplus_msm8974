...
static ssize_t direction_store(struct device *pdev,
...
static ssize_t pwm_enable_store(struct device *pdev,
...
static ssize_t pwm_change_speed_store(struct device *pdev,
...
static void CameraMotor_running(struct work_struct *work)
{
	...
	if (motor_devdata->data.pwm_enable) {
		...
		gpio_set_value(motor_devdata->md_dir_gpio,
			       motor_devdata->data.md_dir);
		...
		motor_devdata->data.pwm_count =
		    motor_devdata->data.md_angle * mdmode * GEAR_RATIO_10 /
		    (10 * ANGEL_PER_PULSE);
...
static int cameramotor_dt(struct device *dev,
			  struct CameraMotor_platform_data *pdata)
{
	...
	motor_devdata->md_vref_gpio =
	    of_get_named_gpio(np, "qcom,platform-vref-gpio", 0);
	if (!gpio_is_valid(motor_devdata->md_vref_gpio))
		pr_err("%s:%d, md-vref-gpio gpio not specified\n", __func__,
		       __LINE__);

	motor_devdata->md_sleep_gpio =
	    of_get_named_gpio(np, "qcom,platform-sleep-gpio", 0);
	if (!gpio_is_valid(motor_devdata->md_sleep_gpio))
		pr_err("%s:%d, md_sleep_gpio gpio not specified\n", __func__,
		       __LINE__);

	motor_devdata->md_dir_gpio =
	    of_get_named_gpio(np, "qcom,platform-dir-gpio", 0);
	if (!gpio_is_valid(motor_devdata->md_dir_gpio))
		pr_err("%s:%d, md_dir_gpio	gpio not specified\n", __func__,
		       __LINE__);

	motor_devdata->md_m0_gpio =
	    of_get_named_gpio(np, "qcom,platform-m0-gpio", 0);
	if (!gpio_is_valid(motor_devdata->md_m0_gpio))
		pr_err("%s:%d, md_m0_gpio gpio not specified\n", __func__,
		       __LINE__);

	motor_devdata->md_m1_gpio =
	    of_get_named_gpio(np, "qcom,platform-m1-gpio", 0);
	if (!gpio_is_valid(motor_devdata->md_m1_gpio))
		pr_err("%s:%d, md_m1_gpio gpio not specified\n", __func__,
		       __LINE__);

	motor_devdata->md_step_gpio =
	    of_get_named_gpio(np, "qcom,platform-step-gpio", 0);
...
static int CameraMotor_probe(struct i2c_client *client,
			     const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		pdata = devm_kzalloc(&client->dev,
				     sizeof(struct CameraMotor_platform_data),
				     GFP_KERNEL);
		if (!pdata) {
	...
	wake_lock_init(&motor_suspend_wake_lock, WAKE_LOCK_SUSPEND,
		       "motor_suspend_lock");
