...
#ifdef CONFIG_PIC1503_FASTCG
#define FASTCG_REQUEST_IRQ_INTERVAL		8000
#endif
#endif
...
static int bq27541_battery_soc(struct bq27541_device_info *di, int time)
{
	...
	if (time != 0) {
		if (soc < di->soc_pre) {
			...
			// allow capacity decrease 1%
			// every 10 minutes when sleeping
...
static int bq27541_battery_resume(struct i2c_client *client)
{
	...
	/* update pre capacity when sleep time is more than 1 minutes */
