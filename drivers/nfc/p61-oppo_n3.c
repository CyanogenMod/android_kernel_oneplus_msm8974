...
ssize_t sendFrame(struct file *filp, const char __user data[], char mode,
		  int count);
ssize_t sendChainedFrame(struct file *filp, const unsigned char __user data[],
			 int count);
...
void receiveAndCheckChecksum(struct file *filp, short rPcb, short rLen,
...
int receive(struct file *filp, unsigned char **data, int len,
	    unsigned char mode);
...
respData_t *receiveChainedFrame(struct file * filp, short rPcb, short rLen)
{
	...
	if (apdbuff == NULL) {
		NFC_ERR_MSG(KERN_ALERT
			    "receiveChainedFrame 2-KMALLOC FAILED!!!\n");
		return NULL;
	}
	...
	if (apdbuff->data == NULL) {
		NFC_ERR_MSG(KERN_ALERT
			    "receiveChainedFrame 3-KMALLOC FAILED!!!\n");
		return NULL;
	}
	    ...
	    unsigned char mode)
{
	...
	if (count_status == 0 && len != 0) {
		NFC_ERR_MSG(KERN_ALERT
			    "ERROR:Failed to receive data from device\n");
		return -1;
	}
			    ...
			    size_t count, loff_t * offset)
{
	...
	NFC_DBG_MSG(KERN_ALERT
		    "p61_dev_read - aquried mutex - calling spi_read \n");
	...
	NFC_DBG_MSG(KERN_ALERT
		    "************  gpio already high read data Test11 *****************\n");
#endif
...
static int __devexit p61_remove(struct spi_device *spi)
{
	...
	NFC_DBG_MSG("Entry : %s\n", __FUNCTION__);
	NFC_DBG_MSG(KERN_INFO " %s ::  name : %s ", __FUNCTION__,
