...
static int m1120_i2c_read(struct i2c_client *client, u8 reg, u8 * rdata,
			  u8 len);
static int m1120_i2c_get_reg(struct i2c_client *client, u8 reg, u8 * rdata);
static int m1120_i2c_write(struct i2c_client *client, u8 reg, u8 * wdata,
			   u8 len);
			   ...
			   u8 len)
{
	...
	if (len > M1120_I2C_BUF_SIZE) {
		mxerr(&client->dev, "i2c buffer size must be less than %d",
		      M1120_I2C_BUF_SIZE);
		return -EIO;
	}
	for (i = 0; i < len; i++)
...
static int m1120_set_power(struct device *dev, bool on)
{
	...
	if (on) {
		if (regulator_count_voltages(p_data->power_vdd) > 0) {
			rc = regulator_set_voltage(p_data->power_vdd, 2700000,
						   2850000);
			if (rc) {
...
static void m1120_motor_stop_hanle(bool need_stop_motor)
{
	...
	m1120_set_detection_mode(&p_m1120_data->client->dev,
...
{
	input_report_key(p_m1120_data->input_dev, M1120_EVENT_BACKWARD_BLOCK,
			 1);
	input_sync(p_m1120_data->input_dev);
	input_report_key(p_m1120_data->input_dev, M1120_EVENT_BACKWARD_BLOCK,
			 0);
...
{
	m1120_data_t *p_data =
	    container_of((struct delayed_work *)work, m1120_data_t, work);
	...
	} else if (p_m1120_data->irq_source == 0)	// polling mode
	{
		...
		if (rear_state != p_m1120_data->last_state) {
			...
			m1120_update_interrupt_threshold(&p_m1120_data->
							 client->dev);

			//printk("Polling report ! rear_state = %d\n", rear_state);

			input_report_key(p_data->input_dev,
					 M1120_EVENT_HALL_STATE, 1);
			input_sync(p_data->input_dev);
			input_report_key(p_data->input_dev,
					 M1120_EVENT_HALL_STATE, 0);
			input_sync(p_data->input_dev);
		}

		schedule_delayed_work(&p_data->work,
				      msecs_to_jiffies(schedule_work_delay));
	}
}
...
static int m1120_clear_interrupt(struct device *dev)
{
	...
	ret =
	    m1120_i2c_set_reg(p_data->client, M1120_REG_PERSINT,
			      p_data->reg.map.persint | 0x01);

	return ret;
}

void m1120_convdata_short_to_2byte(u8 opf, short x, unsigned char *hbyte,
				   unsigned char *lbyte)
{
...
short m1120_convdata_2byte_to_short(u8 opf, unsigned char hbyte,
				    unsigned char lbyte)
{
	short x;
...
static int m1120_update_interrupt_threshold(struct device *dev)
{
	...
	if (p_data->reg.map.intsrs & M1120_VAL_INTSRS_INTTYPE_BESIDE) {
		if (rear_state == 0) {
			m1120_convdata_short_to_2byte(p_data->reg.map.opf, 511,
						      &hthh, &hthl);
			m1120_convdata_short_to_2byte(p_data->reg.map.opf,
						      p_data->thrlow, &lthh,
						      &lthl);
		} else {
			m1120_convdata_short_to_2byte(p_data->reg.map.opf,
						      p_data->thrhigh, &hthh,
						      &hthl);
			m1120_convdata_short_to_2byte(p_data->reg.map.opf, -512,
						      &lthh, &lthl);
		}
	} else {
...
static int m1120_set_operation_mode(struct device *dev, int mode)
{
	...
	case OPERATION_MODE_POWERDOWN:
		...
		mxinfo(&client->dev,
		       "operation mode :OPERATION_MODE_POWERDOWN");
		break;
	case OPERATION_MODE_MEASUREMENT:
		...
		ret = m1120_i2c_set_reg(client, M1120_REG_OPF, opf);
		mxinfo(&client->dev,
		       "operation mode :OPERATION_MODE_MEASUREMENT");
		break;
	case OPERATION_MODE_FUSEROMACCESS:
		...
		ret = m1120_i2c_set_reg(client, M1120_REG_OPF, opf);
		mxinfo(&client->dev,
		       "operation mode :OPERATION_MODE_FUSEROMACCESS");
		break;
	}
...
static int m1120_set_detection_mode(struct device *dev, u8 mode)
{
	...
	if (mode & M1120_DETECTION_MODE_INTERRUPT) {
		if (!p_data->irq_enabled) {
			data =
			    p_data->reg.
			    map.intsrs | M1120_DETECTION_MODE_INTERRUPT;
			err =
			    m1120_i2c_set_reg(p_data->client, M1120_REG_INTSRS,
					      data);
			if (err)
				return err;
			...
			/* enable irq */
			err =
			    request_irq(p_data->irq, &m1120_irq_handler,
					IRQ_TYPE_LEVEL_LOW, M1120_DRIVER_NAME,
					0);
			if (err < 0)
...
static int m1120_reset_device(struct device *dev)
{
	...
	err =
	    m1120_i2c_set_reg(p_data->client, M1120_REG_SRST,
			      M1120_VAL_SRST_RESET);
	if (err) {
		mxerr(&client->dev, "sw-reset was failed(%d)", err);
		return err;
	}
	...
	if (err) {
		mxerr(&client->dev, "m1120_set_operation_mode was failed(%d)",
		      err);
		return err;
	}
...
static int m1120_measure(m1120_data_t * p_data, short *raw)
{
	...
	if (buf[0] & 0x01) {
		adc =
		    m1120_convdata_2byte_to_short(p_data->reg.map.opf, buf[2],
						  buf[1]);
	} else {
...
static long m1120_misc_dev_ioctl(struct file *file, unsigned int cmd,
				 unsigned long arg);
static ssize_t m1120_misc_dev_read(struct file *filp, char *buf, size_t count,
				   loff_t * ofs);
static ssize_t m1120_misc_dev_write(struct file *filp, const char *buf,
				    size_t count, loff_t * ofs);
static unsigned int m1120_misc_dev_poll(struct file *filp,
				 ...
				 unsigned long arg)
{
	...
	case RHALL_IOCTL_SET_CALIBRATION:	//nv write to kernel
		...
		if (copy_from_user(&cali_data, argp, sizeof(cali_data)))
			return -EFAULT;
		if (cali_data.cali_flag != 1
		    || cali_data.lowthd > cali_data.highthd) {
	...
	case RHALL_IOCTL_GET_DATA:	//get hall state
		kbuf = rear_state;
		if (copy_to_user(argp, &kbuf, sizeof(caldata)))
			return -EFAULT;
	...
	case RHALL_IOCTL_SET_REG:
		if (copy_from_user(&kbuf, argp, sizeof(kbuf)))
			return -EFAULT;
		dbg("RHALL_IOCTL_SET_REG([0x%02X] %02X",
		    (u8) ((kbuf >> 8) & 0xFF), (u8) (kbuf & 0xFF));
	...
	case RHALL_IOCTL_SET_INTERRUPT:
		...
		if (kbuf) {
			...
			m1120_set_detection_mode(&p_m1120_data->client->dev,
						 M1120_DETECTION_MODE_POLLING);
		}
		break;
	case RHALL_IOCTL_GET_INTERRUPT:
		kbuf =
		    (p_m1120_data->reg.
		     map.intsrs & M1120_DETECTION_MODE_INTERRUPT) ? 1 : 0;
...
static DEVICE_ATTR(adc, S_IRUGO | S_IWUSR | S_IWGRP, m1120_adc_show, NULL);
static DEVICE_ATTR(set_reg, S_IWUSR | S_IWGRP, NULL, m1120_set_reg_store);
static DEVICE_ATTR(thd, S_IRUGO | S_IWUSR | S_IWGRP, m1120_thd_show, NULL);
...
static ssize_t rotor_node_read(struct file *file, char __user * buf,
...
static ssize_t start_block_detection(struct file *file, const char __user * buf,
...
int m1120_i2c_drv_probe(struct i2c_client *client,
			const struct i2c_device_id *id)
{
	...
	if (p_data->igpio != -1) {
		...
		if (err < 0) {
			mxerr(&client->dev,
			      "gpio_direction_input was failed(%d)", err);
			goto error_2;
		}
	}

	printk("%s  irq:%d  irq-gpio:%d \n", __FUNCTION__, p_data->irq,
	       gpio_get_value(p_data->igpio));
	...
	err =
	    sysfs_create_group(&p_data->input_dev->dev.kobj,
	...
	if (err) {
		mxerr(&client->dev, "m1120_set_detection_mode was failed(%d)",
		      err);
		return err;
	}
...
error_4:
	sysfs_remove_group(&p_data->input_dev->dev.kobj,
			   &m1120_attribute_group);
