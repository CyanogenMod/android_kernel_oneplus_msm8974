...
struct zstats {
	...
	unsigned long		files_active;
	unsigned long		pages_alloced;
	unsigned long		pages_active;
};

static struct zstats zstats;
...
static int zdata_realloc(struct zdata *zdata, loff_t size)
{
	...
	if (size == 0) {
		...
		zdata->z_map = NULL;
		zdata->z_mapsize = 0;
		mutex_lock(&zstats.lock);
		zstats.pages_alloced -= zdata->z_mapsize;
		mutex_unlock(&zstats.lock);
		return 0;
	}
	...
	if (mapsz > zdata->z_mapsize) {
		...
		mutex_lock(&zstats.lock);
		zstats.pages_alloced += mapsz;
		zstats.pages_active += mapsz;
		mutex_unlock(&zstats.lock);
...
static int zfile_proc_stats_show(struct seq_file *p, void *v)
{
	seq_printf(p, "files_opened=%lu\n"
		      "files_active=%lu\n"
		      "pages_alloced=%lu\n"
		      "pages_active=%lu\n",
		      zstats.files_opened, zstats.files_active,
		      zstats.pages_alloced, zstats.pages_active);

	return 0;
}

static int zfile_proc_data_show(struct seq_file *p, void *v)
{
	struct zdata *cur;
...
static int zfile_proc_open_show(struct seq_file *p, void *v)
{
	struct zinfo *cur;

	mutex_lock(&zinfo_lock);
	list_for_each_entry(cur, &z_info, z_node) {
		seq_printf(p, "%s\n", cur->z_data->z_filename);
	}
	mutex_unlock(&zinfo_lock);
...
static int zfile_proc_stats_open(struct inode *inode, struct file *file)
{
	return single_open(file, zfile_proc_stats_show, NULL);
}

static int zfile_proc_data_open(struct inode *inode, struct file *file)
{
	return single_open(file, zfile_proc_data_show, NULL);
}

static int zfile_proc_files_open(struct inode *inode, struct file *file)
{
	return single_open(file, zfile_proc_open_show, NULL);
}

static ssize_t zfile_proc_nowrite(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	return -EINVAL;
}

static const struct file_operations zfile_proc_stats_ops = {
	.owner		= THIS_MODULE,
	.open		= zfile_proc_stats_open,
	.read		= seq_read,
	.write		= zfile_proc_nowrite,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static const struct file_operations zfile_proc_data_ops = {
	.owner		= THIS_MODULE,
	.open		= zfile_proc_data_open,
	.read		= seq_read,
	.write		= zfile_proc_nowrite,
	.llseek		= seq_lseek,
	.release	= single_release,
...
static const struct file_operations zfile_proc_files_ops = {
	.owner		= THIS_MODULE,
	.open		= zfile_proc_files_open,
	.read		= seq_read,
	.write		= zfile_proc_nowrite,
	.llseek		= seq_lseek,
	.release	= single_release,
...
static struct proc_dir_entry *zfile_proc_dir;
static struct proc_dir_entry *zfile_proc_stats;
static struct proc_dir_entry *zfile_proc_data;
static struct proc_dir_entry *zfile_proc_files;

static int zfile_init(void)
{
	memset(&zstats, 0, sizeof(zstats));
	mutex_init(&zstats.lock);

	zfile_proc_dir = proc_mkdir_mode("zfile", 0555, NULL);
	zfile_proc_stats = proc_create_data("stats", 0555, zfile_proc_dir, &zfile_proc_stats_ops, NULL);
	zfile_proc_data = proc_create_data("data", 0555, zfile_proc_dir, &zfile_proc_data_ops, NULL);
	zfile_proc_files = proc_create_data("files", 0555, zfile_proc_dir, &zfile_proc_files_ops, NULL);
